...
public class BackgroundBuildingGroup
{
    public GameObject parentGroup; // Assign "background building 1", etc.
}
...
public class ConnectivitySlider : MonoBehaviour
{
    ...
    void Start()
    {
        foreach (GameObject plot in LeftPlots)
        {
            originalLeftPlotPositions[plot] = plot.transform.position;
        }

        foreach (GameObject plot in RightPlots)
        {
            originalRightPlotPositions[plot] = plot.transform.position;
        }
    }
        ...
        foreach (var group in backgroundBuildingGroups)
        {
            ...
            foreach (Transform child in groupRoot.transform)
            {
                ...
                foreach (GameObject intersection in IntersectionInstances)
                {
                    ...
                    Bounds buildingBounds = GetRendererBounds(child.gameObject);
                    buildingBounds.Expand(-0.5f); // Shrink bounds as much as safely possible
    ...
    Bounds GetRendererBounds(GameObject obj)
    {
        ...
        return new Bounds(obj.transform.position, Vector3.one * 0.1f); // Fallback: very small bounds
    }
            ...
            while (needsAdjustment && iteration < maxIterations)
            {
                ...
                foreach (GameObject street in IntersectionInstances)
                {
                    ...
                    if (Mathf.Abs(plotWorldZ - streetWorldZ) <= (plotSize / 2 + streetSize / 2))
                    {
                        ...
                        if (plotWorldZ >= streetWorldZ)
                            plot.transform.position += Vector3.forward * plotSize;
                        else
                            plot.transform.position += Vector3.back * plotSize;
    ...
    void ResetPlots()
    {
        ...
        {
            if (originalLeftPlotPositions.ContainsKey(plot))
                plot.transform.position = originalLeftPlotPositions[plot];
        }

        foreach (GameObject plot in RightPlots)
        {
            if (originalRightPlotPositions.ContainsKey(plot))
                plot.transform.position = originalRightPlotPositions[plot];
        }
    }
