...
public class FakeInteriorShaderGUI : ShaderGUI
{
    ...
    void SetEnableKeyword(string keyword, bool enabled, Material targetMat, MaterialProperty[] properties)
    {
        var prop = FindProperty(keyword.Replace("_ON", ""), properties, false);
        if (prop != null)
            prop.floatValue = enabled ? 1 : 0;
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        {
            SetEnableKeyword(KEYWORD_LIGHTMAPS, lightmaps, targetMat, properties);

            if (isHDRP)
            {
                SetEnableKeyword(APHHA_TEST, alphaCutoff, targetMat, properties);
            }

            SetEnableKeyword(KEYWORD_ADD_SKYBOX, addSkybox, targetMat, properties);

            SetEnableKeyword(KEYWORD_TILED, tiled, targetMat, properties);

            SetEnableKeyword(KEYWORD_FLIPBOOK, flipbook, targetMat, properties);

            SetEnableKeyword(KEYWORD_FLIPBOOK_INTERNAL_PLANE, flipbookIntPlane, targetMat, properties);
        }
        ...
        if (glassPropertiesFold)
        {
            ...
            var glassNormalProp = FindProperty("_GlassNormal", properties, false);
            if (glassNormalProp != null)
                materialEditor.TextureProperty(glassNormalProp, "Normal Texture");

            var glassReflStrProp = FindProperty("_GlassReflectionStrength", properties, false);
            if (glassReflStrProp != null)
                materialEditor.FloatProperty(glassReflStrProp, "Reflection Strength");

            var glassRefrStrengthProp = FindProperty("_GlassRefractionStrength", properties, false);
            if (glassRefrStrengthProp != null)
                materialEditor.FloatProperty(glassRefrStrengthProp, "Refraction Strength");
        }
        EditorGUILayout.EndFoldoutHeaderGroup();
